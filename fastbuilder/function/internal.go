		...
		FunctionContent: map[string]*FunctionChainItem {
			"": &FunctionChainItem {
				...
				Content: func(env *environment.PBEnvironment,_ []interface{}) {
					...
					env.CommandSender.SendCommand(fmt.Sprintf("execute at @a[name=\"%s\"] run testforblock ~ ~ ~ air",env.RespondUser),configuration.ZeroId)
				},
			},
			...
			"end": &FunctionChainItem {
				...
				Content: func(env *environment.PBEnvironment,_ []interface{}) {
					env.CommandSender.SendSizukanaCommand("gamerule sendcommandfeedback true")
					env.CommandSender.SendCommand(fmt.Sprintf("execute at @a[name=\"%s\"] run testforblock ~ ~ ~ air",env.RespondUser),configuration.OneId)
				},
			},
		},
	})
